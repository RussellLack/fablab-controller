...
import {
  ...
  commitClientsCsv,
  type PreviewResult
} from '@/server/actions/client-import';

/**
 * Client component for /clients/import.
 *
 * UX: textarea + file input → "Preview" runs the server action and
 * shows the planned outcome (inserts / updates / errors) without
 * writing. The user reviews, then "Commit import" runs the same
 * parse + writes the rows.
 *
 * File upload is a thin convenience — we read the file as text and
 * drop it into the textarea so the visible source-of-truth is one
 * place.
 */
export function ClientImportForm() {
  ...
  function runPreview() {
    ...
    startTransition(async () => {
      const result = await previewClientsCsv(csv);
      setPlan(result);
    });
  }
  ...
  function runCommit() {
    if (!csv.trim()) return;
    if (!confirm(t('csv_import.confirm'))) return;
    startTransition(async () => {
      const result = await commitClientsCsv(csv);
      setPlan(result);
    ...
    <>
      ...
      <div className="mb-6">
        <h1 className="text-[22px] font-semibold tracking-tighter">
          {t('csv_import.title_clients')}
        </h1>
        <p className="text-ink-2 text-[13px] mt-1 max-w-2xl">
          {t('csv_import.intro_clients')}
        </p>
      </div>
      ...
      <div className="card mb-4">
        ...
        <textarea
          ...
          rows={10}
          placeholder={t('csv_import.placeholder_clients')}
                ...
                <table className="w-full text-[12px]">
                  <thead className="text-[10px] uppercase tracking-wider text-ink-3 bg-bg">
                    <tr>
                      ...
                      <th className="text-left px-3 py-1.5">{t('csv_import.col_org')}</th>
                      ...
                      return (
                        <tr key={item.rowNumber} className="border-t border-line">
                          ...
                          <td className="px-3 py-1.5 font-mono text-ink-2">
                            {item.parsed?.orgNumber ?? item.raw[2] ?? ''}
                          </td>
